...
func New(clientset kubernetes.Interface, selector, homerAddress, heplifyArgs string) *Sidekick {
	return &Sidekick{
		Clientset:    clientset,
		Selector:     selector,
		HomerAddress: homerAddress,
		HeplifyArgs:  heplifyArgs,
	}
}
...
func (s *Sidekick) createHeplifyPodSpec(targetPod *corev1.Pod) *corev1.Pod {
	...
	// Parse the heplify arguments string
	args := strings.Split(s.HeplifyArgs, " ")
	// Add the homer server address, which is managed separately
	args = append(args, "-hs", s.HomerAddress)
